...
payContractUnstable :: Contract () PaySchema Text ()
payContractUnstable = do
    pp <- endpoint @"pay"
...
payTrace x y =  do
    ...
    callEndpoint @"pay" h $ PayParams (pubKeyHash (walletPubKey (Wallet 2))) x 

    void $ Contract.waitNSlots 1

    callEndpoint @"pay" h  $ PayParams (pubKeyHash (walletPubKey (Wallet 2))) y 
