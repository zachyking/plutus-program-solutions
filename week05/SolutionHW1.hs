...
mkPolicy pkh deadline ctx = traceIfFalse "owner's signature missing or time passed" checkSigAndDeadline 
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    checkSigAndDeadline :: Bool
    checkSigAndDeadline = txSignedBy info pkh  && from deadline `contains` txInfoValidRange info
