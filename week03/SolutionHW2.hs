...
mkValidator pbk deadline _ ctx = traceIfFalse "beneficiary's signature missing" checkSigAndDeadline 
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    checkSigAndDeadline :: Bool
    checkSigAndDeadline = pbk `elem` txInfoSignatories info  && from deadline `contains` txInfoValidRange info
