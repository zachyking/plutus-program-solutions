...
mkValidator dat () ctx =
    traceIfFalse "beneficiary's signature missing" checkSigAndDeadline 
  ...
  where
    ...
    checkSigAndDeadline = (beneficiary1 dat `elem` txInfoSignatories info && to (deadline dat) `contains` txInfoValidRange info) || (beneficiary2 dat `elem` txInfoSignatories info  && from (deadline dat) `contains` txInfoValidRange info)
